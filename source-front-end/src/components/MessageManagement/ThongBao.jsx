...
const ThongBao = (props) => {
    const { SendMessage, editable } = props;
    ...
    useEffect(() => {
        if (editable.enable) {
            setTime(moment(editable.message.scheduled).format("YYYY-MM-DDTHH:mm"));
            setAccessToken(editable.message.accessToken);
            setMessageText(editable.message.textMessage);
            setUrlImage(editable.message.imageUrl);
            setTenThongBao(editable.message.messageName);
        }
        else {
            Reset();
        }
